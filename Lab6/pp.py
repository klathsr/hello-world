...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._build_ui()
        # ไม่ auto load — ให้กด Load JSON เองถ้าต้องการโหลดข้อมูล
    ...
    def _auto_load(self):
        """โหลด tasks.json อัตโนมัติตอนเปิดแอป ถ้ามีไฟล์"""
        if os.path.exists(AUTO_SAVE_PATH):
            try:
                ...
                pass  # ถ้าไฟล์เสียก็ข้ามไป

    def _auto_save(self):
        """บันทึก tasks.json อัตโนมัติ"""
        try:
    ...
    def _on_card_done(self, card: TaskCard):
        ...
        card.deleteLater()
        self._refresh_empty_label()
        self._auto_save()  # บันทึกอัตโนมัติทันทีที่กด Done
    ...
    def _open_add_dialog(self):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted and dlg.result_task:
            ...
            self.cards_layout.addWidget(card)
            self._refresh_empty_label()
            self._auto_save()  # บันทึกอัตโนมัติตอนเพิ่ม task ใหม่
